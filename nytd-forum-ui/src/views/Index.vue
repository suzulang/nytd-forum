    ...
    <section class="bg-zinc-50">
        <div class="container px-6 py-10 mx-auto">
            <div class="grid grid-cols-1 gap-8 mt-8 md:mt-16 md:grid-cols-2">
                <div v-for="article in articleStore.articles" :key="article.id" class="lg:flex">
                    <img class="object-cover w-full h-56 rounded-lg lg:w-64" :src="article.coverImg" :alt="article.title">

                    <div class="flex flex-col justify-between py-6 lg:mx-6">
                        <router-link :to="`/article/detail/${article.id}`" class="text-xl font-semibold text-gray-800 hover:underline dark:text-white">
                            {{ article.title }}
                        </router-link>
                        
                        <span class="text-sm text-gray-500 dark:text-gray-300">On: {{ formatDate(article.createTime) }}</span>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <p class="bg-zinc-50" style="height: 5%;bottom: 0px;width: 100%;text-align: center;">
        <a class="banquan" style="color:#000;"
        href="https://wap.miit.gov.cn/"
        target="_blank">苏ICP备2023004938号-1</a>
    </p>
</template>
